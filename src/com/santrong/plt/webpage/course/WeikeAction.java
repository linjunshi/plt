...
//		request.setAttribute("param", param);
	...
	public String detail(@PathVariable String id) {
		...
		if (MyUtils.isNotNull(weike.getUnitId())) {
			...
			// 其他相关的微课
			List<CourseItem> weikeList = weikeDao.selectWeikeBySameKnowledges(id, 0, 7);//获取当前微课中，含有相同知识点的微课(默认设置查询第一页（从0开始）、查询条数:10)
			request.setAttribute("weikeList", weikeList);
			
			// 同单元的微课
			List<CourseItem> weikeUnitList = weikeDao.selectWeikeByUnitId(weike.getUnitId(), 0, 6);//获取同一单元的微课
