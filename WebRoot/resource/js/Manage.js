...
//					return !treeNode.isLastNode;//最后节点不能显示修改按钮
		...
		syllabus : function(){
			...
			// 显示隐藏微课列表
			$(".school_con").toggleShow($(".school_con ul"), {event : "click", hide : false});
			
			// 显示隐藏展开图标
			$(".school_con").click(function(){
				$(".school_con_hover").each(function(){
					$(this).removeClass("school_con_hover");
				});
				$(this).addClass("school_con_hover");
			});
			
			// 购买
			$(".buy").click(function(){
				var courseId = $(this).prev("input[name=weikeId]").val();
		...
		historyList : function(){
			...
			$(".coll_course").click(function(){
				var courseId = $(this).parents("p").children("input[name=weikeId]").val();
			...
			$(".remove_history").click(function(){
				var attendId = $(this).parents("p").children("input[name=attendId]").val();
				var url = Globals.ctx + "/history/removeAttendHistory"
				$.post(url, {attendId : attendId}, function(result) {
					if (result == "success") {
						location.reload();
					}else {
						Boxy.alert("<i class='error'></i><span>删除失败 !</span>");
